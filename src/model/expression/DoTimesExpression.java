...
public class DoTimesExpression extends ScopedExpression {
    ...
    public void convert (List<String> cmdList) throws SlogoException {
        ...
        variableExpression =
                new VariableExpression(new ArrayList<String>(cmdList.subList(1, 2)), model); // assuming
                                                                                             // just
                                                                                             // on
                                                                                             // variable
        List<String> limits =
                new ArrayList<String>(cmdList.subList(openBracketIndex + 2, closeBracketIndex));
    ...
    public List<TurtleCommand> createTurtleCommands (TurtleCommand turtleCmd) throws SlogoException {
        ...
        NumberExpression limit = (NumberExpression) limitExpression.get(0).evaluate().get(0); // Assuming
                                                                                              // get
                                                                                              // a
                                                                                              // NumberExpression

        NumberExpression variableNumber =
                (NumberExpression) localVariables.get(variableExpression.getId());
        ...
        while (variableNumber.getNumber() < limit.getNumber()) {
            ...
            localVariables.put(variableExpression.getId(),
                               variableNumber.sum(new NumberExpression(1, model)));
